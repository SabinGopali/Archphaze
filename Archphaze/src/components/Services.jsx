...
export default function Services() {
  ...
  return (
    <div className="relative w-full py-20 px-4 lg:px-20 overflow-hidden">
        ...
        <Slider {...settings}>
          {data.map((item, index) => (
            <div key={index} className="p-4">
              <div
                className="group bg-white p-8 rounded-2xl shadow-lg border-b-[6px] border-red-400 
                flex flex-col items-center text-center h-full transition duration-500 transform 
                hover:-translate-y-4 hover:rotate-[1deg] hover:shadow-2xl"
                style={{ minHeight: '420px' }}
              >
                <div className="bg-red-500 p-5 rounded-full mb-6 shadow-md transition hover:bg-red-600">
      ...
      <div className="block sm:hidden relative z-10 space-y-6">
        {data.map((item, index) => (
          <div
            key={index}
            className="bg-white p-8 rounded-2xl shadow-lg border-b-[6px] border-red-400 
            flex flex-col items-center text-center transition duration-500 transform 
